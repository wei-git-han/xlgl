...
public List<Object> getExcleDate() {
    ...
    try {  
        // 同时支持Excel 2003、2007  
        File excelFile = new File("d:/工作簿1.xlsx"); // 创建文件对象  
        ...
        for (Row row : sheet) {
        	/**
        	 * 创建list来存储读取wps的值
        	 * 1：军委办件号；2：文件标题；3、承办人姓名；4、批示指示时间；5、发表批示内容
        	 * 6、督办落实情况人员姓名；7、督办落实情况时间；8、督办落实情况内容
        	 * 9、办理状态名字； 10、承办单位；11、承办人员姓名；12承办人员电话
        	 * 13、印发时间；14、文件号；15、工作分工内容
        	 */
...
//                }                
                ...
                for(String condition:conditionArray) {  
                	...
                	pszsnrMap.put("dblsqkryxm", detailConditions[0]);//6、督办落实情况人员姓名；
                	pszsnrMap.put("dblsqksj", detailConditions[1]);//7、督办落实情况时间；
                	pszsnrMap.put("dblsqknr", detailConditions[2]);//8、督办落实情况内容 
                	dblsqkLis.add(pszsnrMap);
                }
                dataLis.add(3, pszsnrLis);//督办落实情况
