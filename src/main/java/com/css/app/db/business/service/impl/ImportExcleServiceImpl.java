...
public class ImportExcleServiceImpl implements ImportExcleService {
				...
				try {
					// 第一行获取标题来判断DocTypeId
						...
						case "军委首长批示指示督办落实情况表":
							docTypeId = "2";
							packDatasForJWSZ(docTypeId, totalLis, sheet);
							break;
						...
						case "装备发展部领导批示指示督办落实情况表":
							docTypeId = "4";
							packDatasForBLDPS(docTypeId, totalLis, sheet);
							break;
	 ...
	 * @param sheet
	 */
	private void packDatasForJWSZ(String docTypeId, List<List<Object>> totalLis, Sheet sheet) {
		for (Row row : sheet) {
			List<Object> dataLis = new ArrayList<Object>(15);// 用于存储插入数据库
			List<String> resultLis = new ArrayList<String>(15);// 用户存储读取excle数据
			int rowNum = row.getRowNum();
	 ...
	 * @param sheet
	 */
	private void packDatasForBLDPS(String docTypeId, List<List<Object>> totalLis, Sheet sheet) {
		for (Row row : sheet) {
			List<Object> dataLis = new ArrayList<Object>(15);// 用于存储插入数据库
			List<String> resultLis = new ArrayList<String>(15);// 用户存储读取excle数据
			int rowNum = row.getRowNum();
