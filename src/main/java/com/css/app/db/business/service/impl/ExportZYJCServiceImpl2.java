 ...
 */
@Service
public class ExportZYJCServiceImpl2 implements ExportService {
	@Override
	public FileInputStream exportWPSdoc(List<Map<String, String>> list, String fileName, int banjieNum,
		...
		final String TEMPLATENAME = "党中央、中央军委、国务院重要决策部署分工落实情况表.xml";
		...
		String security = list.get(0).get("security")==null?"X 密":list.get(0).get("security");
			...
			if("3".equals(docTypeId))
				title="党中央、中央军委、国务院重要决策部署分工落实情况表";
			else if("5".equals(docTypeId))
				title="装备发展部重要工作分工落实情况表";
