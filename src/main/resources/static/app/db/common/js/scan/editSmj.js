...
function PageInit() {
	...
	var varItem = new Option("科图扫描仪", "69KU");
	...
	document.getElementById("sel_mode").value = "Gray";
...
//	document.getElementById("right").style.height = (document.getElementById("view1").offsetHeight * 1.2) + "px";
	//扫描仪驱动加载，初始化
	var cookieVal = getCookie("69KU");
	if ((deviceName == null || deviceName == '' || deviceName.length <= 0) ||  (cookieVal == null || cookieVal == '' || cookieVal.length <= 0)) {
		if (fScan) {
			fScan = false;
		}
...
//	} else if (status == -5) {
//		dialogDiv.innerHTML = "<div style='text-align:center;line-height:100px'>"
//			+ GetStr(language, "Scanner Connecting") + "...</div>";
	} else {
		dialogDiv.innerHTML = "<div style='text-align:center;line-height:100px'>"
...
function PageMoreSmyInit() {
	...
	document.getElementById("sel_mode_v2").value = "Gray";
...
function getSmwjStreamData(){
	...
	if (imgNames != null && imgNames != '' && imgNames.length > 0) {
		...
		}else if (document.forms["form"]) {//电子保密室扫描保存
			newbootbox.alert2("正在保存中，请稍候...");
			delSmtpTmpFile();
			var dstjSmwjForm = setInterval(function(){
				var smwjVal = document.getElementById("smwj").value;
				if ((smwjVal.split("&").length - 1) == (imgNames.split(',').length - 1)) {
